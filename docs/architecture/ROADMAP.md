...
| CSP                                                                                         | PASS                                | Strict, nonce-based, no `unsafe-inline`/`unsafe-eval`/wildcard sources (`lib/security-headers.ts`). Verified with **zero CSP violations** across a full manual browser walkthrough of every route (login, all six list/detail/new pages, Control record+edit, Resident assign/remove, Research Study's full lifecycle) run against a real production build (`next start`) — that full walkthrough was run **locally**; against the **live Railway deployment specifically**, only login + the first authenticated page (`/patients`) were checked for console/CSP errors and response headers (see the private-network row below) — not the full route list. Required making every route dynamically-rendered (`export const dynamic = "force-dynamic"` on the handful of pages that didn't already have it) — Next's own nonce mechanism only threads through per-request-rendered pages; documented in each page's own comment. Since the CSP logic itself is identical code path regardless of host (`proxy.ts` runs the same way locally and on Railway) this is a low-risk gap, but it is a gap, not "verified against production" — noted here rather than implied.                                                                                                        |
...
| `web → api` private network                                                                 | PASS                                | A real login through the public `https://web-production-c686b1.up.railway.app/login` succeeded and reached `/patients` — this is only possible if `web`'s server reached `api` at `http://cirugias-cruz.railway.internal:3000` (the private domain — note the real Railway service name is `cirugias-cruz`, not `api`; see `deployment-railway.md`) and got a real response back. `api` has no public domain — the browser cannot have reached it directly. `document.cookie` reads empty on the live site (confirms `web_session` is `HttpOnly`, sent correctly over real HTTPS).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
