...
| ---- | ------------------------ | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- | ---------- |
| F-01 | 3 (Patient)              | P1       | Patient carries `email` + `phone` — "no sirve"; must be fully removed, no trace in code/schema                                                      | fixed in code (M11), pending delta re-verify | A1         |
| F-02 | 3 / 3.9 (Patient)        | P1       | Patient age is not computed in the UI; physician should never do the arithmetic                                                                     | fixed in code (M11), pending delta re-verify | B1         |
| F-03 | 5 (Control)              | P1       | Controls have no type/cardinality: need an optional cap — uncapped by default, or "exactly N" + a measurement period (every 12/24/48 h) when capped | fixed in code (M11), pending delta re-verify | A2         |
| F-04 | 3–8 (all tables)         | P2       | Only the name cell is a link; the whole row should navigate                                                                                         | fixed in code (M11), pending delta re-verify | B2         |
| F-05 | 6 / 8 (destructive)      | P1       | Deletes are one-click-after-a-yes/no dialog; need deliberate (type-to-confirm) friction                                                             | fixed in code (M11), pending delta re-verify | B3         |
| F-06 | 5 (Control)              | P2       | Control datetime has no "Now" affordance                                                                                                            | fixed in code (M11), pending delta re-verify | B4         |
| F-07 | 4.5 / 5 (Surgery detail) | P2       | Control history card is mis-placed — should sit directly under the Surgery summary                                                                  | fixed in code (M11), pending delta re-verify | B5         |
| F-08 | 5 (Control)              | P3       | `observations` is required; a Control may legitimately have none                                                                                    | fixed in code (M11), pending delta re-verify | A4         |
| F-09 | 2 (Configuración)        | P2       | CustomField / control-scheme definitions can only be added, not edited or removed                                                                   | fixed in code (M11), pending delta re-verify | A3         |
