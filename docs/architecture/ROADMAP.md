...
- **Milestone 10 — visual/design-system redesign** — **done** (both
  halves now complete). Navigation IA merged earlier (four sections,
  Surgery/Control nested under Patient). The visual half:
  [ADR 0024](../decisions/0024-visual-design-direction.md) design tokens
  (teal/navy palette, Roboto via `next/font`, light-only), restyled
  `components/ui/*` + new primitives (`badge`, `empty-state`, `spinner`,
  `skeleton`, `pending-button`, `PageHeader`, `Breadcrumbs`,
  `ConfirmSubmit`), and the **per-screen 7-principles audit** across
  every route: `loading.tsx` on every list/detail segment, `EmptyState`
  on every zero-row list, `PendingButton` on every form, `ConfirmSubmit`
  `<dialog>` on every destructive action, breadcrumbs + shared
  `PageHeader` throughout. Refs: `docs/design/design-system.md`,
  `docs/design/ux-principles.md` (tracker filled), the `ux-laws` skill.
  Still **not** part of the MVP line — it landed as post-MVP polish.
- **English consolidation** — English is the primary language. All UI
  copy moved into a typed `packages/web/src/messages/en.ts` module
  (the i18n extraction point); nav is Patients / Staff / Research /
  Settings. The product name "Seguimiento de Cirugías" stays as a proper
  noun. Full i18n (locale catalogs + a translation loader) is a later,
  separate effort — see `docs/design/ux-principles.md` § "Future i18n".
...
>
> **Revised again** (this pass): the **physician-facing IA reorganization**
> (Milestone 10's navigation half — the four-section structure
> Pacientes / Plantilla / Investigaciones / Configuración) moved from
> "raised, not scoped" to **MVP-required** by explicit product owner
> decision. The flat backend-mirroring navbar does not satisfy "usable by
> a physician." Only the navigation/IA reorganization is in the MVP line;
> the visual/design-system redesign (Milestone 10's other half) stays
> out of it, blocked on a design direction.
...
- Register **and retrieve** a Procedure Type. Its structure is
  `name` + `description` only — surgical **technique** is a
  `SURGERY`-scoped `ENUM` CustomField, not a ProcedureType attribute
  (Milestone 8.8, ADR 0022; the free-text `technique` field is removed).
...
- **Navigation organized by clinical workflow, not by backend resource** —
  the four-section IA (Pacientes with Surgeries → Controls nested under a
  patient; Plantilla for Residents; Investigaciones; Configuración for
...
- **Post-walkthrough alignment (Milestone 11)** — the fixes the product
  owner's 2026-09-09 walkthrough made a condition of MVP sign-off:
  Patient carries no contact PII and shows a computed age
  (ADR 0025); control definitions with an optional cap + measurement
  period, editable Procedure Type schemes that freeze once they hold
  data, and optional Control observations (ADR 0026/0027); and the web
  UX the owner called non-negotiable — whole-row table links and
  deliberate type-to-confirm deletion. See
  [`milestone-11-alignment-design.md`](milestone-11-alignment-design.md).
...
| Patient (register + retrieve)                                                                  | ✅     | ✅          | ✅          | ✅        | ✅       | ⚠️  | ❌        | UI built (Milestone 8). Walkthrough 2026-09-09 (F-01/F-02): `email` + `phone` must be removed entirely and a computed age shown — Milestone 11 / ADR 0025, not started                                                                                                                                                                                                                    |
...
| Surgery + Control history (register/record/modify + retrieve)                                  | ✅     | ✅          | ✅          | ✅        | ✅       | ⚠️  | ❌        | UI built (Milestone 8). Walkthrough 2026-09-09 (F-07/F-08): reorder the Surgery-detail cards, make Control `observations` optional — Milestone 11 / ADR 0026, not started                                                                                                                                                                                                                  |
...
| CustomField (define on Procedure Type; record/retrieve values on Surgery/Control)              | ✅     | ✅          | ✅          | ✅        | ✅       | ⚠️  | ❌        | Complete through `web` (Milestone 8.6, ADR 0018/0019/0020). Walkthrough 2026-09-09 (F-09): definitions are add-only — schemes must be editable, frozen once they hold data — Milestone 11 / ADR 0027, not started                                                                                                                                                                          |
| Control schemes & cardinality — control definitions with an optional cap + measurement period (**MVP-required**) | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | Milestone 11 / ADR 0026 — walkthrough 2026-09-09 (F-03); not started at any layer                                                                                                                                                                                                                                                                                                       |
...
**Status**: `COMPLETED` for the Physician workflow — backend and
`packages/web` UI both built and tested. One deliberate follow-up gap
remains (Resident-authored Control CustomField input — see end of this
entry).

What's done in `packages/web` (commit `040f3b2`, on `main`): the
Configuración section already carried the "define a CustomField on a
Procedure Type" form; on top of that, the definitions table now shows
**Type** and **Rules** as separate columns (an `ENUM` was previously
indistinguishable from `TEXT`); a shared `CustomFieldValueInputs`
renders one input per physician-defined field, driven entirely by the
Procedure Type's definitions, on the Surgery-registration form
(`SURGERY` scope, reacting to the procedure-type picker) and the
record-Control form (`CONTROL` scope); the Server Actions re-read the
definitions server-side and coerce each value by its `valueType` before
POSTing (`api` stays the sole authority on constraints); recorded values
are shown on the Surgery detail card and each Control row, resolved to
the field name (+ unit for `NUMBER`). Also in this pass: ADR 0020
removed `magnitude` and made `unit` an optional part of the `NUMBER`
constraint (commit `0d6d10b`) — the definition form and schema were
simplified accordingly, and a migration
(`20260905120000_customfield_unit_numeric_only_drop_magnitude`) was
applied to the real Railway Postgres.

What's done in the backend: `CustomField` (`packages/domain/src/shared/custom-field.ts`)
...
**Status**: `COMPLETED` (branch `feat/technique-as-customfield`).
`MVP-required`. See [ADR 0022](../decisions/0022-surgical-technique-is-a-customfield.md).

**What was built**: `ProcedureType.technique` removed across every layer
(same shape as the ADR 0020 `magnitude` removal) —
`ProcedureType.create` / `reconstitute` / `modify` and
`registerProcedureType` / `modifyProcedureType` now take
`name` + `description` only; Prisma column dropped by migration
`20260906130000_drop_procedure_type_technique` (applied to the Railway
Postgres); `serializeProcedureType`, the wire DTO, the `web` register /
edit forms and the list column all lose `technique`. Surgical technique
is now modelled as a `SURGERY`-scoped `ENUM` CustomField (ADR 0018) — the
...
as every other CustomField. No data migration of old `technique` values —
...
re-verification of every route had also been run directly against the
live deployment — Login, Procedure Type, Patient, Surgery, Control
record+edit, Resident register+assign, Research Study's full lifecycle
(create → add-surgery → edit → `DRAFT → IN_PROGRESS → COMPLETED` →
reopen), logout, protected-route redirect — zero console/CSP errors, test
data cleaned up after (see the Milestone 8 closure entry's updated
CSP/private-network rows above).
...
**Status**: `COMPLETED` (post-MVP polish — never in the MVP line). Both
halves done:
...
- **Visual / design-system redesign — not MVP, DONE.** Direction:
  [ADR 0023](../decisions/0023-product-name-seguimiento-de-cirugias.md)
  ...
  Roboto via `next/font`; derived semantic colors; light-only — the
  `prefers-color-scheme` dark block is removed). Delivered: design tokens
  in `globals.css`; `components/ui/*` restyle + primitives (`badge`,
  `empty-state`, `spinner`, `skeleton`, `pending-button`, plus
  `components/{PageHeader,Breadcrumbs,ConfirmSubmit,DashboardNav}`); the
  **per-screen 7-principles audit** across every route group —
  `loading.tsx` on every list/detail segment, `EmptyState` on every
  zero-row list, `PendingButton` on every form, a `ConfirmSubmit`
  `<dialog>` on every destructive action, breadcrumbs + a shared
  `PageHeader` throughout. English is the primary language: all UI copy
  moved to `packages/web/src/messages/en.ts` (the i18n extraction
  point), nav is Patients / Staff / Research / Settings. Working refs:
  `docs/design/design-system.md`, `docs/design/ux-principles.md` (tracker
  filled), the `ux-laws` skill. Deferred: field-level (per-input)
  validation errors (product decision); full i18n (locale catalogs +
  loader) — see `ux-principles.md` § "Future i18n".
...
### Milestone 11 — Post-walkthrough alignment (MVP-required)

**Objective**: close every alignment finding from the Milestone 9
walkthrough (run with the product owner on 2026-09-09) so the walkthrough
can be re-run and signed — the MVP-exit gate.

**Why it exists**: the walkthrough confirmed the core workflow is
completable end-to-end, and produced nine restructuring directives
(F-01…F-09) recorded in
[`alignment-restructuring.md`](alignment-restructuring.md). Four are P1
(patient contact PII, patient age, control cap, deliberate delete) and
gate sign-off; the rest are P2/P3 and land in the same milestone.

**Scope** (full design:
[`milestone-11-alignment-design.md`](milestone-11-alignment-design.md)):

- **WP1** — remove `email`/`phone` from Patient entirely + a Prisma
  column drop; Patient stops composing `Person` (Physician/Resident
  unchanged, Resident keeps `phone`); compute + show patient age in the
  UI. [ADR 0025](../decisions/0025-patient-carries-no-contact-pii.md).
- **WP2** — `packages/web` only: whole table rows become links; a
  type-to-confirm `DangerousConfirm` replaces the bare yes/no dialog on
  data-destroying actions.
- **WP3** — control definitions on a Procedure Type with an optional
  occurrence rule (`uncapped` default, or `capped` = exactly N +
  a measurement period every H hours/days/weeks); the `≤ N` cap enforced
  as a `Surgery` invariant; a computed follow-up "recorded / next due"
  projection on the Surgery read; schemes become editable but a
  definition freezes once it has recorded data; Control `observations`
  becomes optional; the "Now" datetime affordance; the Surgery-detail
  card reorder.
...
- **WP4** — reconcile DOMAIN.md / this ROADMAP / the design docs; re-run
  the Milestone 9 walkthrough with the product owner and fill its
  sign-off.

**Explicitly out of scope**: per-control-definition CustomField sets
(deferred, product-owner-confirmed); reminders/notifications on a missed
timepoint (ADR 0008); any change to Physician/Resident contact fields;
Surgery/Control delete-path changes; Platform Admin; CI/CD.

**Definition of Done**: the completion criteria in
`milestone-11-alignment-design.md` §0 — the PII grep is clean, the cap
and freeze invariants hold with tests, a Control persists with no
observations, every list row navigates, no destructive action completes
without a typed confirmation, the full quality gate + Playwright
`full-workflow` are green, and the Milestone 9 walkthrough sign-off reads
**Passed**.

**Dependencies**: Milestones 8–10; the Milestone 9 walkthrough having
been run (done, 2026-09-09).

**Status**: `NOT_STARTED` — ADRs 0025–0027 and the design doc are
written and product-owner-approved (2026-09-09); implementation not begun.
...
Milestones 1–7 (DONE, deployed)
     ...
     Milestones 4 + 5 + 6 ──▶ Milestone 8 (Frontend: Next.js App Router, BFF)
        ...
        Milestone 7 (private network to api) ───┤
                          ...
                          both DONE on their own branches):
                          8.7 Patient `dni` + search (ADR 0021);
                          8.8 technique → ENUM CustomField (ADR 0022)
                                    │
                                    ▼
                          ...
                          Milestone 10 visual/design-system half — DONE
                          (post-MVP polish; ADR 0023/0024)
```

**Sequential (hard)**: Milestones 1–3 → {4, 5, 6, 7} → 8 → 8.5 → 8.6 →
10 (nav) → {8.7, 8.8} → 9. 8.7 and 8.8 were independent; both have landed
on branches (`feat/patient-dni-and-search`, `feat/technique-as-customfield`).
...
decision). Milestone 10's **visual/design-system half** is **done** as
post-MVP polish (ADR 0023/0024 — palette, Roboto, per-screen 7-principles
audit, English consolidation) and was never in the MVP line.
...
> **CURRENT MILESTONE: 11 — post-walkthrough alignment. Milestone 9's human walkthrough was run on 2026-09-09 with the product owner: the full workflow completed unaided with no P0, and it produced nine alignment directives (F-01…F-09) now scoped as Milestone 11 (ADRs 0025/0026/0027, `milestone-11-alignment-design.md`). Milestone 9's formal sign-off is deferred to the end of Milestone 11 (a short delta re-run). Milestones 1–8.8 and Milestone 10's nav/IA half are all `COMPLETED` and merged. Milestone 10's visual/design-system half is NOT in the MVP line and is done as post-MVP polish (ADR 0023/0024).**
...
- **`PrismaSurgeryRepository.save()` participant-persistence lost-update
  ...
  save added. Controls and CustomField values are immune: no domain
  operation ever removes one, so they persist by `upsert`-per-id with no
  destructive `deleteMany` — only the participant set has a domain
  removal (`removeResidentFromSurgery`), so only it needs the wipe. The
  `upsert` trick therefore cannot simply be copied to participants: a
  correct fix needs a _targeted_ delete (diff), not per-id upsert alone.

  **Explicitly deferred (product owner, confirmed this pass): do
  nothing.** The physician-owner is the only actor who can edit a
  Surgery's roster, through one browser, one action at a time — there is
  no concurrent writer, so the race cannot occur. No optimistic locking,
  version column, or participant-diffing logic is introduced.

  **Trigger conditions that would end the deferral** (revisit if any
  becomes true — earliest natural checkpoint is triage after Milestone
  9's walkthrough): (a) more than one authenticated human can mutate the
  same Surgery's participant set — e.g. a Resident-facing "leave surgery"
  action, a second physician device, or shared credentials; (b) any
  non-interactive writer (background job, bulk import, webhook) writes
  Surgery aggregates; (c) the Milestone 9 walkthrough surfaces a
  double-submit / retry race on assign/remove-resident.

  **First fix to reach for when a trigger fires: incremental diff**
  (read current participant rows in `save()`, `createMany` only the
  additions with `skipDuplicates`, `deleteMany` only the
  `(surgeryId, residentId)` pairs that left the in-memory set). It is the
  smallest change that removes the one dangerous property — _silent_
  loss of an unrelated participant with no error. Optimistic locking (a
  `version` column across every Surgery write) is the escalation only if
  concurrent editing of the same aggregate becomes a _designed_ feature;
  a pessimistic `SELECT … FOR UPDATE` is worth it only if the Application
  closure pattern is being reworked anyway (it would need `load` and
